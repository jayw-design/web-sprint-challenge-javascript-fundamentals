...
// The nested function can access the variable internal because the nested function is inside the parent function which allows its subordinate function to access declared variables inside.
...
function animalNames(arry) {
  let result = [];
  arry.forEach(arry => {
    result.push(`name: ${arry.animal_name}, scientific: ${arry.scientific_name}`);
  });
  return result;
}
...
function lowPopulationAnimals(arry) {
  return arry.filter((arry) => {
    return arry.population < 5;
  });
}
...
function USApop(arry) {
  let totalpop = arry.reduce((total, i) => {
...
CuboidMaker.prototype.volume = function () {
  let result = this.length * this.width * this.height;
  return result;
};
